...
class ConversationServer:
    ...
    def __init__(self, router: APIRouter):
        ...
        router.add_api_route(
            "/message/send_with_file", self._send_message_with_file, methods=["POST"]
        )
            ...
            if "multipart/form-data" in content_type:
                ...
                if "message" in form:
                    ...
                    if "file" in form:
                        ...
                        if DEBUG_MODE:
                            print(
                                f"[DEBUG] Server: Received file: {uploaded_file.filename}"
                            )
                        ...
                        if file_text:
                            ...
                            original_text = (
                                message.parts[0].text if message.parts else ""
                            )
...
Please analyze the above resume content and provide a detailed rating."""
                            ...
                            if DEBUG_MODE:
                                print(
                                    f"[DEBUG] Server: Enhanced message with {len(file_text)} characters of extracted text"
                                )
                else:
                    if DEBUG_MODE:
                ...
                try:
                    ...
                    if DEBUG_MODE:
                        print(
                            f"[DEBUG] Server: Unicode decode error - request may contain binary data: {unicode_error}"
                        )
        ...
        return SendMessageWithFileResponse(
            result=MessageInfo(
