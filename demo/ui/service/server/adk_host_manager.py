...
class ADKHostManager(ApplicationManager):
    ...
    def __init__(self, api_key: str = "", uses_vertex_ai: bool = False):
        ...
        self._pending_message_ids: list[str] = (
            []
        )  # Tracks user messages awaiting agent response
    ...
    def update_api_key(self, api_key: str):
        ...
        if api_key and api_key != self.api_key:
            ...
            if not self.uses_vertex_ai:
                ...
                print(
                    "[WARN] Attempted to update API key while using Vertex AI auth. Ignoring."
                )
    ...
    def create_conversation(self) -> Conversation:
        ...
        session.state = {}
        print(
            f"[DEBUG] Initialized state for newly created ADK session {conversation_id} in create_conversation."
        )
    ...
    async def process_message(self, message: Message):
        ...
        if not message_id or not conversation_id:
            print(
                f"[ERROR] process_message: Missing metadata. message_id={message_id}, conversation_id={conversation_id}. Aborting."
            )
            return
        ...
        print(
            f"[INFO] process_message started for message_id: {message_id} in conversation: {conversation_id}"
        )
        ...
        if not any(get_message_id(m) == message_id for m in conversation.messages):
            ...
            print(
                f"[DEBUG] Appended user message {message_id} to conversation {conversation_id}."
            )
        ...
        if message.metadata and "file_path" in message.metadata:
            print(
                f"[DEBUG] ADK Host Manager: file_path in metadata: {message.metadata['file_path']}"
            )
        ...
        if target_agent_url:
            ...
            print(
                f"[INFO] Injecting target_agent_url into state_delta: {target_agent_url}"
            )
        ...
        try:
            ...
            if session is None:
                print(
                    f"[WARN] No session found, creating new session {conversation_id}."
                )
                session = self._session_service.create_session(
                    app_name=self.app_name,
                    user_id=self.user_id,
            ...
            if session.state is None:
                print(
                    f"[WARN] Session {conversation_id} exists but state was None, initializing."
                )
                session.state = {}

            print(
                f"[DEBUG LOCK] State before Call 1: {session.state}, Type: {type(session.state)}, ID: {id(session.state)}"
            )
            ...
            print(
                f"[DEBUG LOCK] State after Call 1: {session.state}, ID: {id(session.state)}"
            )
            ...
            if message.role == "user":
                if session.state is None:
                    print(
                        f"[WARN LOCK] State became None before Call 2. Reinitializing."
                    )
                    session.state = {}
                ...
                print(
                    f"[DEBUG LOCK] State after Call 2: {session.state}, ID: {id(session.state)}"
                )
        ...
        try:
            print(
                f"[INFO] Calling _host_runner.run_async for session {conversation_id}, message {message_id}"
            )
            ...
            async for event in self._host_runner.run_async(
                ...
                if conversation and event.content.role != "user":
                    conversation.messages.append(
                        self.adk_content_to_message(event.content, conversation_id)
                    )
            ...
            if final_agent_event and final_agent_event.author not in [
                "user",
                "host_agent",
            ]:
                print(
                    f"[DEBUG] Processing final event {final_agent_event.id} as agent response."
                )

                if final_agent_event.content.role != "agent":
                    final_agent_event.content.role = "agent"

                agent_response = self.adk_content_to_message(
                    final_agent_event.content, conversation_id
                )
                    ...
                    agent_response.metadata.update(
                        {
                            ...
                            "remote_agent_url": target_agent_url
                            or message.metadata.get("remote_agent_url"),
                        }
                    )

                    if conversation and not any(
                        get_message_id(m) == agent_response.metadata["message_id"]
                        for m in conversation.messages
                    ):
                        conversation.messages.append(agent_response)
                        print(
                            f"[INFO] Appended agent response {agent_response.metadata['message_id']} to conversation {conversation_id}"
                        )
            ...
            if conversation:
                ...
                print(
                    f"[INFO] Appended error message to conversation {conversation_id}"
                )
    ...
    def task_callback(self, task_update: TaskCallbackArg, agent_card: AgentCard):
        ...
        print(
            f"[INFO] task_callback received update: type={type(task_update).__name__}, task_id={task_id}, agent={agent_card.name}"
        )
        ...
        if isinstance(task_update, TaskStatusUpdateEvent):
            current_task_state = self.add_or_get_task(
                task_update
            ...
            self.attach_message_to_task(
                task_update.status.message, current_task_state.id
            )
            ...
            if (
                ...
                print(
                    f"[INFO] task_callback detected COMPLETED status with message for task {current_task_state.id}"
                )
                ...
                if final_message.role != "agent":
                    final_message.role = "agent"

                # Append the final message to the main conversation state
                if conversation:
                    if not any(
                        get_message_id(m) == response_message_id
                        for m in conversation.messages
                    ):
                        conversation.messages.append(final_message)
                        print(
                            f"[INFO] Appended final message {response_message_id} from task_callback/COMPLETED status to conversation {conversation_id}."
                        )
                        ...
                        originating_user_msg_id = next(
                            (
                                msg_id
                                for msg_id, t_id in self._task_map.items()
                                if t_id == current_task_state.id
                            ),
                            None,
                        )
                        if (
                            originating_user_msg_id
                            and originating_user_msg_id in self._pending_message_ids
                        ):
                            self._pending_message_ids.remove(originating_user_msg_id)
                            print(
                                f"[DEBUG] Removed originating user message {originating_user_msg_id} from pending via task_callback completion."
                            )
                    ...
                    print(
                        f"[ERROR] task_callback: No conversation object found for completed task message: {conversation_id}"
                    )
        ...
        # --- Handle Task Artifact Updates ---
            ...
            self.process_artifact_event(
                current_task_state, task_update
            )  # Assemble chunks
        ...
        # --- Handle Full Task Object Updates (Less common for remote agents?) ---
            ...
            existing_task = next(
                filter(lambda x: x.id == current_task_state.id, self._tasks), None
            )
            # Link message<->task and trace IDs
            self.attach_message_to_task(
                current_task_state.status.message, current_task_state.id
            )
            ...
            if not existing_task:
                ...
                print(
                    f"[INFO] Added new task {current_task_state.id} via full Task object in task_callback."
                )
            else:
                self.update_task(current_task_state)
            ...
            if (
                current_task_state.status.state == TaskState.COMPLETED
                and current_task_state.status.message
            ):
                print(
                    f"[INFO] task_callback detected COMPLETED Task object with message for task {current_task_state.id}"
                )
                ...
                if conversation:
                    if not any(
                        ...
                        conversation.messages.append(final_message)
                        print(
                            f"[INFO] Appended final message {response_message_id} from task_callback/COMPLETED Task object to conversation {conversation_id}."
                        )
                        ...
                        if (
                            ...
                            self._pending_message_ids.remove(originating_user_msg_id)
                            print(
                                f"[DEBUG] Removed originating user message {originating_user_msg_id} from pending via task_callback/Task completion."
                            )
                    # else:
                    ...
                    print(
                        f"[ERROR] task_callback: No conversation object found for completed Task object message: {conversation_id}"
                    )

            return current_task_state  # Return the updated/added task state
        else:
            print(
                f"[WARN] Unhandled type received in task_callback: {type(task_update)}"
            )
    ...
    def emit_event(self, task_data: TaskCallbackArg, agent_card: AgentCard):
        ...
        if isinstance(task_data, TaskStatusUpdateEvent):
            if task_data.status.message:
                ...
                if content.metadata:
                    content.metadata.update(base_metadata)
                else:
                    content.metadata = base_metadata
            else:  # Create a simple text event for status change
                content = Message(
                    parts=[
                        TextPart(text=f"Task status changed: {task_data.status.state}")
                    ],
            ...
            content = Message(
                parts=[TextPart(text="Task update received")],
                role="system",
                metadata=base_metadata,
            )
    ...
    def insert_message_history(self, task: Task, message: Message | None):
        ...
        if task.history is None:
            task.history = []  # Initialize if needed
    ...
    def add_or_get_task(self, task_update_data: TaskCallbackArg) -> Task:
        ...
        if not current_task:
            print(
                f"[INFO] add_or_get_task: Task {task_id} not found. Creating new Task state."
            )
            ...
            current_task = Task(
                ...
                status=getattr(
                    task_update_data, "status", TaskStatus(state=TaskState.SUBMITTED)
                ),
                metadata=getattr(
                    task_update_data, "metadata", {"conversation_id": conversation_id}
                ),
    ...
    def process_artifact_event(
        ...
        artifact_index = (
            artifact.index
        )  # Index of the artifact within the task (for chunking)
        ...
        if not artifact.append:  # First chunk (or only chunk)
            if artifact.lastChunk is None or artifact.lastChunk:
                ...
                print(
                    f"[DEBUG] process_artifact_event: Received complete artifact (Index {artifact_index}) for task {task_id}."
                )
                ...
                print(
                    f"[DEBUG] process_artifact_event: Received first chunk for artifact (Index {artifact_index}) for task {task_id}."
                )
            ...
            print(
                f"[DEBUG] process_artifact_event: Received append chunk for artifact (Index {artifact_index}) for task {task_id}."
            )
            if (
                ...
                if artifact.lastChunk:
                    ...
                    print(
                        f"[DEBUG] process_artifact_event: Received last chunk. Finalizing artifact (Index {artifact_index}) for task {task_id}."
                    )
                    ...
                    if not self._artifact_chunks[
                        task_id
                    ]:  # Remove task entry if no more chunks pending
                        del self._artifact_chunks[task_id]
            else:
                print(
                    f"[WARN] process_artifact_event: Received append chunk for unknown/unstored artifact (Task {task_id}, Index {artifact_index}). Ignoring."
                )
    ...
    def register_agent(self, url):
        ...
        try:
            ...
            if not agent_data.url:
                agent_data.url = url  # Ensure URL is stored
        ...
        for part in message.parts:
            if part.type == "text":
                ...
                try:
                    ...
                    print(
                        f"[WARN] Failed to dump data part to JSON: {e}. Sending as text."
                    )
                ...
                if hasattr(part, "file") and part.file:
                    if part.file.uri:
                        ...
                        part.file.bytes
                    ):  # Expects base64 encoded string? ADK expects bytes.
                        try:
                            file_bytes = base64.b64decode(part.file.bytes)
                            parts.append(
                                types.Part.from_data(  # Use from_data for bytes
                                    data=file_bytes, mime_type=part.file.mimeType
                                )
                            )
                        except Exception as e:
                            print(
                                f"[ERROR] Failed to decode base64 bytes for file part: {e}"
                            )
    ...
    def adk_content_to_message(
        ...
        for part in content.parts:
            ...
            if part.function_call:
                ...
                try:
                    ...
                    print(
                        f"[WARN] Error processing potential JSON in text part: {e}. Treating as plain text."
                    )
                    parts.append(TextPart(text=part.text))
        ...
        return Message(
            ...
            metadata={
                "conversation_id": conversation_id
            },  # Ensure conversation ID is attached
        )
    ...
    def _handle_function_response(
        ...
        try:
            ...
            if (
                not part.function_response.response
                or "result" not in part.function_response.response
            ):
                    ...
                    # NEW: Handle artifact reference (specific dict structure)
                        ...
                        try:
                            ...
                            if adk_file_part and adk_file_part.inline_data:
                                ...
                                print(
                                    f"[WARN] Failed to load or get inline_data for artifact {file_id}."
                                )
                                parts.append(DataPart(data=item))  # Fallback
                        except Exception as artifact_e:
                            print(
                                f"[ERROR] Failed to load artifact {item.get('artifact-file-id')}: {artifact_e}"
                            )
                            parts.append(DataPart(data=item))  # Fallback
                    else:
