...
async def SendMessageWithFile(message: Message, file_path: str) -> str | None:
    ...
    try:
        print(
            f"[DEBUG] SendMessageWithFile: Input message metadata: {message.metadata}"
        )
        ...
        print(
            f"[DEBUG] SendMessageWithFile: Final message metadata: {message_with_file.metadata}"
        )

        response = await client.send_message_with_file(
            SendMessageWithFileRequest(params=message_with_file), file_path
        )
    ...
    try:
        ...
        if conversation_id:
            ...
            if state.messages:
                for msg in state.messages:
                    ...
                    if msg.message_id in state.background_tasks:
                        ...
                        print(
                            f"[DEBUG] Preserving background task message: {msg.message_id}"
                        )
                    ...
                    if msg.content and len(msg.content) > 0:
                        content_text = (
                            msg.content[0][0]
                            if isinstance(msg.content[0][0], str)
                            else ""
                        )
                        if "[Uploaded file:" in content_text or "📎" in content_text:
                            should_preserve = True
                            print(
                                f"[DEBUG] Preserving file upload message: {msg.message_id}"
                            )
                    ...
                    if (
                        ...
                        and msg.metadata.get("file_upload", False)
                    ):
                        should_preserve = True
                        print(
                            f"[DEBUG] Preserving marked file upload message: {msg.message_id}"
                        )
            ...
            if not messages:
                ...
                print(
                    f"[DEBUG] No server messages, keeping {len(local_messages)} local messages"
                )
            else:
                # 合并服务器消息和本地未同步消息
                ...
                print(
                    f"[DEBUG] Merged {len(server_messages)} server messages with {len(unique_local_messages)} unique local messages"
                )
        ...
        if p.type == "text":
            ...
            try:
                ...
                if "type" in p.data and p.data["type"] == "form":
                    ...
                    # Check if this is an ADK function call (like send_task) and hide it unless in debug mode
                    debug_mode = os.environ.get("DEBUG_MODE", "").lower() == "true"
                    if (
                        not debug_mode
                        and isinstance(p.data, dict)
                        and "id" in p.data
                        and str(p.data["id"]).startswith("adk-")
                    ):
                        # Skip adding this data part to hide function call logs from UI
                        pass
                    else:
...
async def pick_agent_using_chatgpt(user_message: str) -> str | None:
    ...
    agent_descriptions = "\n".join(
        f"- {agent.name}: {agent.description} ({agent.url})" for agent in remote_agents
    )
...
"""
    ...
    try:
        async with httpx.AsyncClient(timeout=20.0) as client:
            response = await client.post(azure_url, headers=headers, json=body)
            response.raise_for_status()
            data = response.json()
